...
warning('error', 'Octave:broadcast');
  ...
  % return test loss
  testloss = loss(model, datas2{3}, 0);
end
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  % input_to_hid is array[hid i; input i]
  % hid_to_class is array[class i; hid i]
  [hid_input, hid_output, class_input, log_class_prob, class_prob] = forward_pass(model, data);
 
  d_loss_by_d_class_in = -(data.targets - class_prob) ./ size(data.inputs, 2); % [class i; case i]
  d_loss_by_d_hid_out = model.hid_to_class.' * d_loss_by_d_class_in; % [hid i; case i]
  d_loss_by_d_hid_in = d_loss_by_d_hid_out .* hid_output .* (1-hid_output); % the logistic nonlinearity

  ret.hid_to_class = d_loss_by_d_class_in * hid_output.' + wd_coefficient * model.hid_to_class; % [class i; hid i]
  ret.input_to_hid = d_loss_by_d_hid_in * data.inputs.' + wd_coefficient * model.input_to_hid; % [hid i; input i]
end
